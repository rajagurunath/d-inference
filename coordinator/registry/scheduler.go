...
func (r *Registry) commitProviderReservation(
	...
	// would close the pair to online traffic for capacityProbeOutcomeWindow and
	// never resolve it. Batch reservations therefore neither claim the probe nor
	// are rejected by another request's claim. That cannot admit batch through a
	// closed gate: the routing gate (capacityCooled) already skips a pair inside
	// its TTL or holding a fresh probe claim, for every lane.
...
func (r *Registry) snapshotProviderIntoPLockedEx(dst *routingSnapshot, p *Provider, model string, traits RequestTraits, selfRouteOwner bool, ignoreProviderBreaker bool, now time.Time) (bool, GateReason) {
	...
	*dst = routingSnapshot{}
	...
	// the same locked view as the slot counters below (buildCandidateInto
...
func (r *Registry) buildCandidateInto(c *routingCandidate, pr *PendingRequest, now time.Time) (candidateRejection, GateReason, bool) {
	...
	if pr.Traits.Lane == LaneBatch &&
		...
		return rejectCapacity, GateBatchHeadroom, false
	}
